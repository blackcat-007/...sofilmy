...
function Details() {
  ...
  return (
    <div className="relative p-4 mt-4 flex flex-col items-center w-full overflow-hidden rounded-xl">
  {/* ✅ Background Blur */}
  {movie.image && (
    <div
      key={movie.image}
      className="absolute inset-0 bg-cover bg-center blur-2xl scale-110 opacity-70 transition-all duration-700"
  ...
  <div className="bg-black relative z-10 rounded-xl p-6 shadow-lg flex flex-col md:flex-row gap-6 w-full md:w-5/6 items-center md:items-start">
    ...
    <div className="flex-1">
      ...
      <div className="flex items-center gap-4 mt-2">
        <ReactStars size={22} half={true} value={movie.userStars || 0} edit={false} />
        <span className="text-gray-300">⭐ {movie.tmdbRating} / 10 TMDB</span>
      </div>
      ...
      <div>
        ...
        {["flatrate", "rent", "buy"].map((k) => (
          <div key={k} className="mb-2">
            <div className="text-xs uppercase tracking-wide text-gray-400 mb-1">{k}</div>
